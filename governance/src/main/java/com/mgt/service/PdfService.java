...
public class PdfService {
    ...
    //  BIRTH CERTIFICATE
    public byte[] generateBirthCertificate(BirthDeathCertificate b) {
        ...
        try {
            ...
            canvas.setStrokeColor(green).setLineWidth(2)
                  .rectangle(new Rectangle(20, 20, W - 40, H - 40)).stroke();
            canvas.setLineWidth(0.7f)
                  .rectangle(new Rectangle(28, 28, W - 56, H - 56)).stroke();
            canvas.release();
            ...
            doc.add(new Paragraph("BIRTH CERTIFICATE")
                    .setTextAlignment(TextAlignment.CENTER).setFontSize(18).setBold()
                    .setFontColor(ColorConstants.RED).setMarginBottom(10));

            // Certificate No + Issue Date  
            String today  = LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));
            String serial = "BD-" + System.currentTimeMillis();

            Table head = new Table(new float[]{50, 50})
                    .setWidth(UnitValue.createPercentValue(100));

            head.addCell(new Cell()
                    .add(new Paragraph("Certificate No: " + safe(b.getCertificateNo())).setBold())
                    .setBorder(null));
            ...
            addRow(table, "Name",           b.getName(),             green);
            addRow(table, "Date of Birth",  b.getDob(),              green);
            addRow(table, "Place of Birth", b.getPlaceOfBirth(),     green);
            addRow(table, "Gender",         b.getGenderOfBirth(),    green);
            addRow(table, "Father's Name",  b.getFathersName(),      green);
            addRow(table, "Mother's Name",  b.getMothersName(),      green);
            addRow(table, "Address",        b.getPermanentAddress(), green);
            addRow(table, "Mobile",         b.getMobileNumber(),     green);

            doc.add(table);

            // Declaration
            doc.add(new Paragraph(
                    "This is to certify that the above information has been recorded in accordance with official government records.")
                    .setTextAlignment(TextAlignment.CENTER).setFontSize(9)
                    .setFontColor(new DeviceRgb(60, 60, 60)).setMarginTop(10));

            // QR — center, then Signature, then Footer (same order as Citizen)
            doc.add(makeQrImage("ID:" + b.getId() + "|CERT:" + serial + "|NAME:" + b.getName()));
            doc.add(makeSignatureTable(b));
            doc.add(new Paragraph("Verify certificate using QR Code | Madaripur Municipality System")
                    .setTextAlignment(TextAlignment.CENTER).setFontSize(8).setMarginTop(15));
            doc.close();
    ...
    //  DEATH CERTIFICATE
    public byte[] generateDeathCertificate(BirthDeathCertificate b) {
        ...
        try {
            ...
            DeviceRgb navyBlue = new DeviceRgb(20, 40, 90);
            canvas.setStrokeColor(navyBlue).setLineWidth(2)
                  .rectangle(new Rectangle(20, 20, W - 40, H - 40)).stroke();
            canvas.setLineWidth(0.7f)
                  .rectangle(new Rectangle(28, 28, W - 56, H - 56)).stroke();
            canvas.release();
            ...
            doc.add(new Paragraph("DEATH CERTIFICATE")
                    .setTextAlignment(TextAlignment.CENTER).setFontSize(18).setBold()
                    .setFontColor(navyBlue).setMarginBottom(10));

            String today  = LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));
            String serial = "DD-" + System.currentTimeMillis();
            ...
            addRow(table, "Deceased Name",  b.getName(),           navyBlue);
            addRow(table, "Date of Death",  b.getDateOfDeath(),    navyBlue);
            addRow(table, "Place of Death", b.getPlaceOfDeath(),   navyBlue);
            addRow(table, "Gender",         b.getGender(),         navyBlue);
            addRow(table, "NID",            b.getNid(),            navyBlue);
            addRow(table, "Birth Reg. No",  b.getBirthNo(),        navyBlue);
            addRow(table, "Applicant Name", b.getApplicantName(),  navyBlue);
            addRow(table, "Relation",       b.getRelation(),       navyBlue);
            addRow(table, "Address",        b.getAddress(),        navyBlue);
            addRow(table, "Mobile",         b.getMobileNumber(),   navyBlue);

            doc.add(table);

            doc.add(new Paragraph(
                    "This is to certify that the death of the above-named individual has been duly registered " +
                    "in accordance with the official government records of Madaripur Municipality.")
                    .setTextAlignment(TextAlignment.CENTER).setFontSize(9)
                    .setFontColor(new DeviceRgb(60, 60, 60)).setMarginTop(10));
    ...
    private Table makeSignatureTable(BirthDeathCertificate b) {
        Table sign = new Table(new float[]{50, 50})
                .setWidth(UnitValue.createPercentValue(100))
                .setMarginTop(10)
                .setKeepTogether(true);

        sign.addCell(signatureCell(b.getFirstSignature(), b.getFirstSeal(), b.getFirstApprovedBy(), "First Approving Officer", false));
        sign.addCell(signatureCell(b.getSecondSignature(), b.getSecondSeal(), b.getSecondApprovedBy(), "Final Approving Officer", true));
        return sign;
    }
    ...
    private Cell signatureCell(String dataUrl, String sealUrl, String officer, String role, boolean right) {
        ...
        // Do not print officer id/username such as 7 or 5. Only show the role label.
        ...
        // Seal must be below the officer title.
