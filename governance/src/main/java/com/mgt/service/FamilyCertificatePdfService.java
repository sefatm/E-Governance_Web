...
@Service
public class FamilyCertificatePdfService {

    public byte[] generateFamilyCertificate(FamilyCertificate f) {
        ...
        try {
            ...
            doc.add(new Paragraph("FAMILY CERTIFICATE")
                    .setTextAlignment(TextAlignment.CENTER)
                    .setFontSize(18)
                    .setBold()
            ...
            String serial = "FAM-" + System.currentTimeMillis();

            Table head = new Table(new float[]{50, 50})
                    .setWidth(UnitValue.createPercentValue(100));

            head.addCell(new Cell()
                    .add(new Paragraph("Certificate No: " + safe(f.getCertificateNo())).setBold())
                    .setBorder(null));
            ...
            addRow(table, "Head of Family", safe(f.getHeadName()), green);
            addRow(table, "NID", safe(f.getNid()), green);
            addRow(table, "Total Members", safe(f.getMemberCount()), green);
            addRow(table, "Address", safe(f.getAddress()), green);
            addRow(table, "Contact", safe(f.getContact()), green);
            addRow(table, "Purpose", safe(f.getPurpose()), green);

            doc.add(table);

            // Family member details
            if (f.getMembersJson() != null && !f.getMembersJson().isBlank()) {
                try {
                    com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
                    java.util.List<java.util.Map<String, Object>> members = mapper.readValue(
                        f.getMembersJson(), new com.fasterxml.jackson.core.type.TypeReference<java.util.List<java.util.Map<String, Object>>>() {});
                    doc.add(new Paragraph("Family Member Details").setBold().setFontSize(11).setFontColor(green).setMarginTop(10));
                    Table mt = new Table(new float[]{8, 27, 18, 17, 30}).setWidth(UnitValue.createPercentValue(100));
                    String[] heads = {"#", "Name", "Relation", "DOB", "NID / Birth No"};
                    for (String h : heads) mt.addHeaderCell(new Cell().add(new Paragraph(h).setBold().setFontSize(7)).setBackgroundColor(new DeviceRgb(226, 240, 232)).setPadding(4));
                    int i = 1;
                    for (java.util.Map<String, Object> m : members) {
                        mt.addCell(new Cell().add(new Paragraph(String.valueOf(i++)).setFontSize(7)).setPadding(3));
                        mt.addCell(new Cell().add(new Paragraph(memberVal(m, "name", "memberName")).setFontSize(7)).setPadding(3));
                        mt.addCell(new Cell().add(new Paragraph(memberVal(m, "relation", "relationship")).setFontSize(7)).setPadding(3));
                        mt.addCell(new Cell().add(new Paragraph(memberVal(m, "dob", "dateOfBirth")).setFontSize(7)).setPadding(3));
                        mt.addCell(new Cell().add(new Paragraph(memberVal(m, "nid", "birthNo", "nidOrBirthNo")).setFontSize(7)).setPadding(3));
                    }
                    doc.add(mt);
            ...
            doc.add(new Paragraph(
                    "This is to certify that the above-mentioned family resides at the stated address " +
                    "and the provided information is true according to local government records.")
                    .setTextAlignment(TextAlignment.CENTER)
                    .setFontSize(9)
            ...
            // QR
            doc.add(makeQrImage("ID:" + f.getId() + "|CERT:" + serial + "|HEAD:" + f.getHeadName()));

            // Signature
            doc.add(makeSignatureTable(f));
    ...
    private Table makeSignatureTable(FamilyCertificate f) {
        Table sign = new Table(new float[]{50, 50})
                .setWidth(UnitValue.createPercentValue(100)).setMarginTop(18);
        sign.addCell(signatureCell(f.getFirstSignature(), f.getFirstSeal(), f.getFirstApprovedBy(), "First Approving Officer", false));
        sign.addCell(signatureCell(f.getSecondSignature(), f.getSecondSeal(), f.getSecondApprovedBy(), "Final Approving Officer", true));
        return sign;
    }
    ...
    private String memberVal(java.util.Map<String, Object> member, String... keys) {
        for (String key : keys) {
