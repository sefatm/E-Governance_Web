...
@Component({
  selector: 'app-admin-officer',
  templateUrl: './admin-officer.component.html',
  styleUrls: ['./admin-officer.component.css']
})
export class AdminOfficerComponent implements OnInit {
  ...
  load(): void {
    ...
    this.http.get<AppUser[]>(`${this.base}/getall`).subscribe({
      next: (res) => {
        this.users     = res.filter(u => u.role === 'Admin / Municipal Officer');
